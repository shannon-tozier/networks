...
attributes <- filter(attributes, i %in% edges2$i | i %in% edges2$j)
```
...
```{r}
V(net2)$color <- ifelse(attributes$Gi == "F", "lightpink", "lightblue")

V(net2)$shape <- ifelse(attributes$i %in% sub_edges$i, "square", "circle")
